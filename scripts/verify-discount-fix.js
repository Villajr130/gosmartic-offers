...
const CASES = [
  { asin: 'B0DYJR3MTJ', label: 'id 25 (HP Omnibook 5) - discount sin old_price, debe dar 0% (bug corregido)', expectZero: true },
  { asin: 'B0DPHXRZV6', label: 'id 26 (Lenovo IdeaPad Flex 5) - discount sin old_price, debe dar 0% (bug corregido)', expectZero: true },
  { asin: 'B09WX9XBKD', label: 'id 36 (Amazon Echo Pop) - discount sin old_price, debe dar 0% (bug corregido)', expectZero: true },
...
async function main() {
  ...
  for (const { asin, label, expectZero } of CASES) {
    ...
    try {
      ...
      console.log(`old_price:               ${product?.old_price ?? '(ninguno)'}`);
      ...
      console.log(`--> getConfirmedDiscountField(): ${discountField ?? '(null, no hay badge confirmado con old_price)'}`);
